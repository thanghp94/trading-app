...
import {
  ...
  portfolioManager,
  PM_TOOL_NAME,
  HAIKU_MODEL as HAIKU_MODEL_REF,
} from "./agents.js";
...
const CACHE_TTL_MS = Number(
  process.env.COUNCIL_CACHE_TTL_MS ?? 4 * 60 * 60 * 1000,
);
...
function ledgerAdd(
  ledger: CostLedger,
  entry: CostLedger["entries"][number],
): void {
...
export type CouncilRequest = {
  ...
  /** Cache-only fundamentals/ownership lookups for the fundamental analyst. */
...
export async function runCouncil(req: CouncilRequest): Promise<CouncilResult> {
  const ctx = buildContext(req.symbol, req.timeframe, req.alertEngine, {
    getFundamentals: req.getFundamentals,
    getOwnership: req.getOwnership,
  });
  if (!ctx)
    return {
      ok: false,
      error: `no snapshot for ${req.symbol}:${req.timeframe}`,
    };

  const key = cacheKey(req.symbol, req.timeframe, ctx.lastCandleTime);
  const hit = cache.get(key);
  if (hit && hit.expiresAt > Date.now())
    return { ok: true, report: hit.report, cached: true };
  ...
  try {
    ...
    ledgerAdd(ledger, {
      stage: "bull",
      model: HAIKU_MODEL_REF,
      inTok: bullResult.inTok,
      outTok: bullResult.outTok,
      costUsd: bullResult.costUsd,
    });
    ledgerAdd(ledger, {
      stage: "bear",
      model: HAIKU_MODEL_REF,
      inTok: bearResult.inTok,
      outTok: bearResult.outTok,
      costUsd: bearResult.costUsd,
    });
    ...
    const mgrResult = await runPrompt(
      mgrSpec,
      "research-manager",
      controller.signal,
    );
    ledgerAdd(ledger, {
      ...
      model: mgrSpec.model,
      inTok: mgrResult.inTok,
      outTok: mgrResult.outTok,
      costUsd: mgrResult.costUsd,
    });

    // Stage 4: trader (sequential)
    const traderSpec = trader(ctx, mgrResult.text);
    const traderResult = await runPrompt(
      traderSpec,
      "trader",
      controller.signal,
    );
    ledgerAdd(ledger, {
      stage: "trader",
      model: traderSpec.model,
      inTok: traderResult.inTok,
      outTok: traderResult.outTok,
      costUsd: traderResult.costUsd,
    });

    // Stage 5: 3 risk personas in parallel
    const riskRuns = await Promise.all([
      runPrompt(
        riskAggressive(ctx, traderResult.text),
        "risk-aggressive",
        controller.signal,
      ),
      runPrompt(
        riskNeutral(ctx, traderResult.text),
        "risk-neutral",
        controller.signal,
      ),
      runPrompt(
        riskConservative(ctx, traderResult.text),
        "risk-conservative",
        controller.signal,
      ),
    ]);
    ...
    const riskVerdicts: RiskVerdict[] = riskRuns.map((r, i) => {
      ledgerAdd(ledger, {
        stage: riskStages[i],
        model: riskAggressive(ctx, "").model,
        inTok: r.inTok,
        outTok: r.outTok,
        costUsd: r.costUsd,
      });
    ...
    const pmResult = await runPrompt(
      pmSpec,
      "portfolio-manager",
      controller.signal,
    );
    ledgerAdd(ledger, {
      ...
      model: pmSpec.model,
      inTok: pmResult.inTok,
      outTok: pmResult.outTok,
      costUsd: pmResult.costUsd,
    });

    if (!isPMDecision(pmResult.toolInput)) {
      return {
        ok: false,
        error: `portfolio-manager stage failed: tool ${PM_TOOL_NAME} not invoked or bad shape`,
      };
    }
