import type { CouncilContext, AnalystOutput, DebateRound, RiskVerdict } from './types.js';
...
function ctxSnippet(ctx: CouncilContext): string {
  ...
  const bars = ctx.recentCandles
    .slice(-20)
    .map((b) => `${new Date(b.time * 1000).toISOString().slice(0, 16)}: O${b.open} H${b.high} L${b.low} C${b.close} V${Math.round(b.volume)}`)
    .join('\n');
  return (
    `Symbol: ${ctx.symbol} ${ctx.timeframe}\n` +
    `Last close: ${last?.close ?? 'N/A'} at ${last ? new Date(last.time * 1000).toISOString().slice(0, 16) : 'N/A'} UTC\n` +
    `Last 20 candles:\n${bars}\n\n` +
    `Active S/R zones: ${active.length === 0 ? '(none)' : active.map((z) => `${z.bottom.toFixed(4)}..${z.top.toFixed(4)} ${z.type}`).join(', ')}\n` +
    `Active wave: ${activeWave ? `${activeWave.direction} pts: ${activeWave.points.map((p) => `${p.label}@${p.price.toFixed(4)}`).join('→')}` : '(none)'}\n` +
...
export function analystTechnical(ctx: CouncilContext): PromptSpec {
  return {
    model: HAIKU_MODEL, maxTokens: 350,
    system: 'You are a senior technical analyst. Be terse, cite exact prices. 5-8 sentences.',
...
export function bull(ctx: CouncilContext, analysts: AnalystOutput[]): PromptSpec {
  const summaries = analysts.map((a) => `[${a.stage}]: ${a.text.slice(0, 200)}`).join('\n');
  return {
    model: HAIKU_MODEL, maxTokens: 350,
    system: 'You are the BULL advocate in a structured debate. Argue the strongest long case from the analyst reports. Be concrete — cite prices and zones.',
    user: `Analyst reports:\n${summaries}\n\nContext:\n${ctxSnippet(ctx)}\n\nArgue the long case.`,
  };
}

export function bear(ctx: CouncilContext, analysts: AnalystOutput[]): PromptSpec {
  const summaries = analysts.map((a) => `[${a.stage}]: ${a.text.slice(0, 200)}`).join('\n');
  return {
    model: HAIKU_MODEL, maxTokens: 350,
    system: 'You are the BEAR advocate in a structured debate. Argue the strongest short/avoid case from the analyst reports. Be concrete — cite prices and zones.',
    user: `Analyst reports:\n${summaries}\n\nContext:\n${ctxSnippet(ctx)}\n\nArgue the short/avoid case.`,
  };
}

export function researchManager(ctx: CouncilContext, analysts: AnalystOutput[], debate: DebateRound): PromptSpec {
  const summaries = analysts.map((a) => `[${a.stage}]: ${a.text.slice(0, 150)}`).join('\n');
  return {
    model: HAIKU_MODEL, maxTokens: 400,
    system: 'You are the Research Manager. Synthesize the analyst reports and debate into a balanced, evidence-based view. No position recommendation yet.',
    user: `Analysts:\n${summaries}\n\nBull case:\n${debate.bull}\n\nBear case:\n${debate.bear}\n\nContext:\n${ctxSnippet(ctx)}\n\nSynthesize.`,
  };
}
...
export function trader(ctx: CouncilContext, manager: string): PromptSpec {
  return {
    model: HAIKU_MODEL, maxTokens: 350,
    system: 'You are the Trader. Based on the research synthesis, propose ONE concrete trade setup with entry, SL, TP prices and size rationale. Be specific — exact prices.',
    user: `Research synthesis:\n${manager}\n\nContext:\n${ctxSnippet(ctx)}\n\nPropose the trade setup.`,
  };
}

export function riskAggressive(ctx: CouncilContext, proposal: string): PromptSpec {
  return {
    model: HAIKU_MODEL, maxTokens: 300,
    system: 'You are the AGGRESSIVE risk persona (high risk tolerance). Evaluate the trade proposal — lean toward participation. State your verdict clearly.',
...
export function riskNeutral(ctx: CouncilContext, proposal: string): PromptSpec {
  return {
    model: HAIKU_MODEL, maxTokens: 300,
    system: 'You are the NEUTRAL risk persona (balanced risk/reward). Evaluate the trade proposal objectively. State your verdict clearly.',
    user: `Trade proposal:\n${proposal}\n\nContext:\n${ctxSnippet(ctx)}\n\nNeutral risk verdict.`,
  };
}

export function riskConservative(ctx: CouncilContext, proposal: string): PromptSpec {
  return {
    model: HAIKU_MODEL, maxTokens: 300,
    system: 'You are the CONSERVATIVE risk persona (low risk tolerance). Evaluate the trade proposal — prefer capital preservation. State your verdict clearly.',
...
const PM_TOOL_SCHEMA = {
  ...
  properties: {
    ...
    rationale: { type: 'string', description: 'One-paragraph decision rationale' },
  },
};

export const PM_TOOL_NAME = 'submit_decision';

export function portfolioManager(ctx: CouncilContext, proposal: string, risk: RiskVerdict[]): PromptSpec {
  const riskSummaries = risk.map((r) => `[${r.persona}]: ${r.text.slice(0, 200)}`).join('\n');
  return {
    model: SONNET_MODEL, maxTokens: 500,
    system: 'You are the Portfolio Manager. Make the final capital allocation decision using the submit_decision tool. Weigh all risk verdicts and the trade proposal.',
