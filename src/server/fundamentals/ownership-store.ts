...
/** SQLite cache for ownership data (changes slowly → cache hard, refresh nightly). */
export class OwnershipStore {
  ...
  /** Parsed ownership for a symbol, or null when absent. */
  get(symbol: string): Ownership | null {
    const row = this.db
      .prepare("SELECT payload FROM ownership WHERE symbol = ?")
      .get(symbol.toUpperCase()) as { payload: string } | undefined;
    if (!row) return null;
    try {
      return JSON.parse(row.payload) as Ownership;
    } catch {
      return null;
    }
  }

  /** Upsert ownership + stamp fetched_at (seconds). */
  set(symbol: string, ownership: Ownership): void {
    this.db
      .prepare(
        `INSERT INTO ownership (symbol, payload, fetched_at)
         VALUES (?, ?, unixepoch())
         ON CONFLICT(symbol) DO UPDATE SET
           payload = excluded.payload,
           fetched_at = excluded.fetched_at`,
      )
      .run(symbol.toUpperCase(), JSON.stringify(ownership));
  }

  /** Seconds since the symbol was cached, or null if never cached. */
  ageSec(symbol: string): number | null {
    const row = this.db
      .prepare("SELECT fetched_at FROM ownership WHERE symbol = ?")
