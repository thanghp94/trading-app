...
export function App() {
  ...
  useShortcuts({
    nextCell: () =>
      setActiveIdx((i) =>
        layout.cells.length === 0 ? 0 : (i + 1) % layout.cells.length,
      ),
    prevCell: () =>
      setActiveIdx((i) =>
        layout.cells.length === 0
          ? 0
          : (i - 1 + layout.cells.length) % layout.cells.length,
      ),
    setActiveTimeframe: (tf: Timeframe) =>
      activeCell && updateCell(activeCell.id, { timeframe: tf }),
  ...
  return (
    ...
    >
      ...
      <PaperTradingPanel
        ...
        refresh={journal.refresh}
        update={journal.update}
      />
      <StrategyBuilderPanel
        open={dock.activePanel === "strategy"}
        onClose={dock.close}
      />
      <WatchlistPanel
        open={dock.activePanel === "watchlist"}
        onClose={dock.close}
      ...
      />
      <AlertPanel
        open={dock.activePanel === "alerts"}
        onClose={dock.close}
        alerts={alerts}
        onClear={clearAlerts}
      />
      <JournalPanel
        open={dock.activePanel === "journal"}
        onClose={dock.close}
        trades={journal.trades}
        stats={journal.stats}
        update={journal.update}
      ...
      <SymbolSearch
        ...
        onPick={onSymbolSearchPick}
      />
    </div>
  );
}

function DailyPnlBadge({
  pnl,
}: {
  pnl: {
    closedToday: number;
  ...
  if (pnl.closedToday === 0) {
    return (
      <span style={pnlBadgeStyle}>
        <span style={{ color: "#8b949e" }}>Today: no closed trades</span>
      </span>
    );
  }
  ...
  return (
    <span style={pnlBadgeStyle}>
      ...
      <b style={{ color: positive ? "#26a69a" : "#ef5350" }}>
        {positive ? "+" : ""}
        {pnl.rToday.toFixed(2)}R
      </b>
      <span style={{ color: "#8b949e" }}>
        · {pnl.wins}W/{pnl.losses}L · {pnl.closedToday} closed
      </span>
    </span>
  );
}
